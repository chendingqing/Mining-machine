...
class IndexController extends CommonController {
  public function admin_add(){
    ...
    function main(){
	 ...
	 public function configsave(){
		...
		foreach($data as $key=>$value)
		{
			...
			M("config")->where(array("id"=>$key))->save($data);
    ...
    public function df1(){
		...
		$map  = '1 ';
		if($start){
			$map .= " and  addtime >  " . strtotime(date ($start));
		} 
		if($end){
			$map .= " and addtime <  " . strtotime(date ($end));
		} 
    	...
    	$list = $User->where ( $map )->order ( 'addtime  desc' )->limit ( $p->firstRow, $p->listRows )->select ();
		$arr= array();
		if($list){
			$users = M ( 'user' );
			foreach($list as $k => $v){
				...
				$oob = array();
				$oob= $users->where(array('UE_ID'=>$v['uid']))->find();
				$v['p_user'] = $oob['ue_account'];
				// if($k == 1){
				...
				if($v['bid'] > 0){
					$oobs = array();
					$oobs=M('user')->where(array('UE_ID'=>$v['bid']))->find();
					$v['p_name'] = $oobs['ue_account'];
				}else{
					$v['p_name'] = '--';
				}
				$arr[] = $v;
   ...
   public function usercl(){
    	...
    	// dump(I('post.UE_account'));die;
    	 if(M('user')->where(array('UE_account'=>I('post.UE_account')))->save($data)){
			...
			/*  */
			$userdata = M ( 'user' )->where ( array ( 'UE_account' => I('post.UE_account') ) )->find ();
			if($userdata['mfkj'] == 1 && $data['level'] > 0){
				/* wk 后台首次激活  增加免费矿机 ID 210 */
				/*
				...
				$map['kjbh'] = $orderSn;
				$orderform->add($map);*/
				M('user')->where(array('UE_account'=>I('post.UE_account')))->data(array('mfkj'=>2))->save();
				/* wk 增加矿机 ID 210 */
			} 
    ...
    public function admincl(){
    	...
    	if(M('member')->where(array('MB_username'=>I('post.MB_username')))->save($data)){
    		$this->success('修改成功!','/admin8899.php/Home/Index/adminlist');
    	}else{
    		$this->success('修改失败!');
    	}
    }
    ...
    public function adminadd(){
    	...
    	if(M('member')->add($data)){
    		$this->success('添加成功!','/admin8899.php/Home/Index/adminlist');
    	}else{
    		$this->success('添加失败!');
    	}
    	}else{
    ...
    public function admindel(){
    	...
    	if($data<>''&& $userxx['mb_username']<>''){
    		...
    		$this->success('删除成功!','/admin8899.php/Home/Index/adminlist');
    	}else{
    		$this->success('不能删除!');
    	}
...
//插入数据
    public function klineacl(){
    	...
    	if(M('date')->add($data)){
    		$this->success('添加成功!','/admin8899.php/Home/Index/klinea');
    	}else{
    		$this->success('添加失败!');
    	}
    	}else{
