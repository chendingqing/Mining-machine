		...
		if ($_GET ['debug']) {
			...
			if ($rshy) {
				...
				if (!file_exists($user_pic)) {
					...
					$name=$rshy['nickname']; 
					$img = new \Think\Image();
					$img->open($ticket)->thumb(296, 298)->save($ticket);
					if(!empty($logo)){$img->open($logo)->thumb(110, 112)->save($logo);}
					define('THINKIMAGE_WATER_CENTER', 5);
				    $img->open('./card.jpg')->water($ticket, array(165,525))->water($logo, array(474,57))->text($name,'./hei.ttf','20','#d53917', array(-255,-970))->save($user_pic);
				}	
